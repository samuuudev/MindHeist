    ...
    def _make_callback(self, index: int):
        async def callback(interaction: discord.Interaction):
            async with self._lock:
                if self.finished:
                    await interaction.response.send_message("Alguien ya acertó.", ephemeral=True)
                    return

                if interaction.user.id in self._answered_users:
                    await interaction.response.send_message("Ya usaste tu intento en esta Pregunta de Oro.", ephemeral=True)
                    return
                ...
                if is_correct:
                    ...
                    await interaction.response.send_message("Respuesta incorrecta. No tienes más intentos.", ephemeral=True)
...
class GoldCog(commands.Cog):
    ...
    async def gold_scheduler(self):
        ...
        for config in configs:
            ...
            async with self.bot.db.acquire() as conn:
                last_event = await conn.fetchrow("SELECT ended_at FROM gold_events WHERE guild_id = $1 ORDER BY created_at DESC LIMIT 1;", guild_id)
    ...
    async def _launch_gold_event(self, guild: discord.Guild, config):
        ...
        try:
            ...
            if not channel:
                log.info(f"Pregunta de Oro cancelada en {guild.name}: canal de oro no configurado.")
                return

            async with self.bot.db.acquire() as conn:
                jackpot_row = await conn.fetchrow("SELECT COALESCE(SUM(jackpot), 0) AS total_jackpot FROM gold_events WHERE guild_id = $1 AND winner_id IS NULL AND is_active = FALSE;", guild_id)

            accumulated_jackpot = jackpot_row["total_jackpot"] if jackpot_row else 0

            base_reward = random.randint(config["gold_min_points"], config["gold_max_points"])
            total_reward = base_reward + accumulated_jackpot

            question_data = await self.generator.generate(difficulty=random.choice(["medium", "hard"]), category=None)
            ...
            async with self.bot.db.acquire() as conn:
                question_id = await conn.fetchval(
                    "INSERT INTO questions (content, options, correct_index, difficulty, category, source) VALUES ($1, $2::jsonb, $3, $4::question_difficulty, $5::question_category, $6::question_source) RETURNING question_id;",
                    question_data["question"], json.dumps(question_data["options"]), question_data["correct_index"],
                    question_data.get("difficulty", "medium"), question_data.get("category", "general"), question_data.get("source", "openai")
                )

                event_id = await conn.fetchval(
                    "INSERT INTO gold_events (guild_id, question_id, reward_points, jackpot, is_active, started_at) VALUES ($1, $2, $3, $4, TRUE, NOW()) RETURNING event_id;",
                    guild_id, question_id, total_reward, base_reward
                )

            hype_embed = discord.Embed(
                title="Pregunta de Oro inminente",
                description=(f"En **5 segundos** aparecerá una Pregunta de Oro.\n\nRecompensa: **{total_reward} puntos**" + (f"\nIncluye jackpot acumulado de **{accumulated_jackpot}** puntos." if accumulated_jackpot > 0 else "") + "\n\nSolo el primero en acertar gana. Un intento por persona."),
            ...
            gold_embed = discord.Embed(title="Pregunta de Oro", description=f"**{question_data['question']}**", color=discord.Color.gold())
            gold_embed.add_field(name="Recompensa", value=f"**{total_reward} puntos**", inline=True)
            gold_embed.add_field(name="Tiempo", value="**60 segundos**", inline=True)
            gold_embed.add_field(name="Reglas", value="1 intento por persona. Primer acierto gana.", inline=True)
            if accumulated_jackpot > 0:
                gold_embed.add_field(name="Jackpot acumulado", value=f"+{accumulated_jackpot} puntos extra incluidos", inline=False)
            ...
            if view.winner_id:
                async with self.bot.db.acquire() as conn:
                    ...
                    await conn.execute("UPDATE gold_events SET winner_id = $1, is_active = FALSE, ended_at = NOW(), jackpot = 0 WHERE event_id = $2;", view.winner_id, event_id)
                    await conn.execute("UPDATE gold_events SET jackpot = 0 WHERE guild_id = $1 AND winner_id IS NULL AND is_active = FALSE;", guild_id)

                participants = len(view._answered_users)
                winner_embed = discord.Embed(title="Pregunta de Oro ganada", description=(f"**{view.winner_name}** ha acertado la Pregunta de Oro.\n\n**+{total_reward} puntos**\nRespondió en **{view.response_time:.1f}s**\nParticipantes: **{participants}**"), color=discord.Color.gold())
                await channel.send(embed=winner_embed)

                if logger:
                    winner_member = guild.get_member(view.winner_id)
                    await logger.log_gold(guild_id=guild_id, winner=winner_member, reward=total_reward, participants=participants, jackpot_accumulated=0)

            else:
                async with self.bot.db.acquire() as conn:
                    await conn.execute("UPDATE gold_events SET is_active = FALSE, ended_at = NOW() WHERE event_id = $1;", event_id)

                new_jackpot = accumulated_jackpot + base_reward
                participants = len(view._answered_users)
                correct_answer = question_data["options"][question_data["correct_index"]]

                jackpot_embed = discord.Embed(title="Nadie acertó la Pregunta de Oro", description=(f"La respuesta correcta era: **{correct_answer}**\n\nParticipantes: **{participants}**\n\nEl jackpot se acumula.\nJackpot actual: **{new_jackpot} puntos**"), color=discord.Color.dark_gold())
                await channel.send(embed=jackpot_embed)

                if logger:
                    await logger.log_gold(guild_id=guild_id, winner=None, reward=base_reward, participants=participants, jackpot_accumulated=new_jackpot)

            log.info(f"Pregunta de Oro #{event_id} en {guild.name} — {'Ganada' if view.winner_id else 'Sin ganador'}")
    ...
    @app_commands.command(name="gold", description="Información sobre las Preguntas de Oro y el jackpot actual")
    async def gold_info(self, interaction: discord.Interaction):
        ...
        async with self.bot.db.acquire() as conn:
            ...
            last_event = await conn.fetchrow("SELECT * FROM gold_events WHERE guild_id = $1 ORDER BY created_at DESC LIMIT 1;", guild_id)

            stats = await conn.fetchrow("SELECT COUNT(*) AS total_events, COUNT(winner_id) AS won_events, COUNT(*) - COUNT(winner_id) AS no_winner FROM gold_events WHERE guild_id = $1 AND is_active = FALSE;", guild_id)

            top_gold = await conn.fetch("SELECT username, gold_wins FROM users WHERE guild_id = $1 AND gold_wins > 0 ORDER BY gold_wins DESC LIMIT 5;", guild_id)
        ...
        embed = discord.Embed(title="Pregunta de Oro — Información", color=discord.Color.gold())

        if jackpot > 0:
            embed.add_field(name="Jackpot acumulado", value=f"**{jackpot} puntos**\nSe sumará a la próxima Pregunta de Oro.", inline=False)
        else:
            embed.add_field(name="Jackpot", value="Sin jackpot acumulado.", inline=False)
        ...
        if last_event:
            ...
            time_str = (f"Hace {mins_ago} min" if mins_ago < 60 else f"Hace {mins_ago // 60}h {mins_ago % 60}m")

            embed.add_field(name="Última Pregunta de Oro", value=f"{last_info}\n{time_str}", inline=True)

        if stats and stats["total_events"] > 0:
            win_rate = (stats["won_events"] / stats["total_events"]) * 100
            embed.add_field(name="Estadísticas", value=(f"Total: **{stats['total_events']}**\nGanadas: **{stats['won_events']}** ({win_rate:.0f}%)\nSin ganador: **{stats['no_winner']}**"), inline=True)

        if top_gold:
            top_lines = []
            for i, row in enumerate(top_gold):
                top_lines.append(f"{i + 1}. {row['username']} — **{row['gold_wins']}** victorias")
            embed.add_field(name="Top cazadores de Oro", value="\n".join(top_lines), inline=False)

        embed.add_field(
            name="Funcionamiento",
            value=(f"Aparece aleatoriamente cada {config['gold_interval_min']}-{config['gold_interval_max']} min.\n"
    ...
    @app_commands.command(name="forcegold", description="[Admin] Forzar una Pregunta de Oro")
    @app_commands.default_permissions(administrator=True)
    async def force_gold(self, interaction: discord.Interaction):
        guild_id = interaction.guild_id

        if self._active_events.get(guild_id, False):
            await interaction.response.send_message("Ya hay una Pregunta de Oro activa.", ephemeral=True)
            return
        ...
        if not config["gold_channel_id"]:
            await interaction.response.send_message("No hay canal de oro configurado. Usa `/setup gold_channel:#canal` primero.", ephemeral=True)
            return

        await interaction.response.send_message("Pregunta de Oro forzada. Aparecerá en unos segundos.", ephemeral=True)
