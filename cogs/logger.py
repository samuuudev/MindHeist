...
class LoggerCog(commands.Cog):
    ...
    async def _get_log_channel(self, guild_id: int) -> discord.TextChannel | None:
        """Obtiene el canal de logs del servidor. Devuelve None si no existe."""
    ...
    async def send_log(
        ...
        """
        Envía un embed de log al canal configurado.

        Args:
            guild_id: ID del servidor.
            log_type: Categoría del evento.
            title: Título del log.
            description: Descripción del evento.
            fields: Lista de dicts con 'name', 'value' e 'inline'.
            user: Usuario que ejecutó la acción.
            target: Usuario objetivo de la acción.
        """
    ...
    async def log_quiz(
        ...
        correct: bool, points: int, difficulty: str,
        category: str, response_time: float,
    ):
        result = "Correcto" if correct else "Incorrecto"
        await self.send_log(
            guild_id=guild_id,
            log_type=LogType.QUIZ,
            title=f"Quiz completado — {result}",
            description=f"{user.mention} respondió un quiz.",
            fields=[
    ...
    async def log_daily(
        self, guild_id: int, user: discord.Member,
        correct: bool, points: int, streak: int,
    ):
        title = (
            f"Daily completado — Racha: {streak} días"
            if correct
            else "Daily fallado — Racha perdida"
        )
        await self.send_log(
            guild_id=guild_id,
            log_type=LogType.DAILY,
            title=title,
            description=f"{user.mention} completó su pregunta diaria.",
            fields=[
    ...
    async def log_gold(
        self, guild_id: int, winner: discord.Member | None,
        reward: int, participants: int, jackpot_accumulated: int,
    ):
        if winner:
        ...
        if not winner:
            fields.append({
                ...
                "value": f"{jackpot_accumulated} pts",
            })

        await self.send_log(
            guild_id=guild_id,
            log_type=LogType.GOLD,
            title=title,
            description=desc,
            fields=fields,
            user=winner,
    ...
    async def log_robbery(
        self, guild_id: int, attacker: discord.Member,
        victim: discord.Member, success: bool, amount: int,
    ):
        if success:
            title = "Robo exitoso"
            ...
            title = "Robo fallido"
            desc = (
                f"{attacker.mention} intentó robar a {victim.mention} "
                f"y perdió **{amount}** monedas."
            )

        await self.send_log(
            guild_id=guild_id,
            log_type=LogType.ROBBERY,
            title=title,
            description=desc,
            fields=[
    ...
    async def log_shield(
        self, guild_id: int, user: discord.Member,
        duration: str, cost: int,
    ):
        await self.send_log(
            guild_id=guild_id,
            log_type=LogType.ECONOMY,
            title="Escudo comprado",
            description=f"{user.mention} compró un escudo de **{duration}**.",
            fields=[
    ...
    async def log_admin_give(
        self, guild_id: int, admin: discord.Member,
        target: discord.Member, points: int, money: int,
    ):
        await self.send_log(
            guild_id=guild_id,
            log_type=LogType.ADMIN,
            title="Modificación manual de economía",
            description=f"{admin.mention} modificó los recursos de {target.mention}.",
            fields=[
    ...
    async def log_admin_reset(
        self, guild_id: int, admin: discord.Member,
        target_type: str, member: discord.Member | None = None,
    ):
    ...
    async def log_role_change(
        ...
        role: discord.Role, action: str, reason: str,
    ):
        await self.send_log(
            guild_id=guild_id,
            log_type=LogType.SYSTEM,
            title=f"Rol {action}",
            description=(
                f"Rol **{role.name}** {action} a {member.mention}.\n"
                f"Razón: {reason}"
            ),
            user=member,
        )
