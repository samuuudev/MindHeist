...
class ConfirmView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.admin_id:
            await interaction.response.send_message("Solo el administrador puede confirmar.", ephemeral=True)
            return
        self.confirmed = True
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.admin_id:
            await interaction.response.send_message("Solo el administrador puede cancelar.", ephemeral=True)
            return
        self.confirmed = False
        button.disabled = True
...
def validate_param(parameter: str, value: str) -> tuple[float | int, str]:
    ...
    if parameter == "gold_quiz_chance":
        ...
        if not (rules["min"] <= num <= rules["max"]):
            raise ValueError(f"Debe estar entre {rules['min']} y {rules['max']}")
        display = f"{num}%"
...
class AdminCog(commands.Cog):
    ...
    # ── /setup ─────────────────────────────────────────────────
    @app_commands.command(name="setup", description="[Admin] Configuración inicial del bot")
    @app_commands.default_permissions(administrator=True)
    @app_commands.describe(quiz_channel="Canal para quizzes", gold_channel="Canal para Preguntas de Oro", log_channel="Canal de logs del bot", top1_role="Rol para el Top 1", top2_role="Rol para el Top 2", top3_role="Rol para el Top 3")
    async def setup(self, interaction: discord.Interaction, quiz_channel: discord.TextChannel | None = None, gold_channel: discord.TextChannel | None = None, log_channel: discord.TextChannel | None = None, top1_role: discord.Role | None = None, top2_role: discord.Role | None = None, top3_role: discord.Role | None = None):
        ...
        embed = discord.Embed(title="Configuración actualizada", color=discord.Color.green())
        ...
        if not any([quiz_channel, gold_channel, log_channel, top_roles]):
            embed.description = ("No se modificó nada. Especifica al menos un parámetro.\nEjemplo: `/setup gold_channel:#canal`")

        embed.set_footer(text="Usa /config para ver la configuración completa")
        await interaction.response.send_message(embed=embed, ephemeral=True)

    # ── /config ────────────────────────────────────────────────
    @app_commands.command(name="config", description="[Admin] Ver la configuración actual del bot")
    @app_commands.default_permissions(administrator=True)
    async def config(self, interaction: discord.Interaction):
        guild_id = interaction.guild_id

        async with self.bot.db.acquire() as conn:
            cfg = await conn.fetchrow("SELECT * FROM guild_config WHERE guild_id = $1", guild_id)

        if not cfg:
            await interaction.response.send_message("No hay configuración para este servidor. Usa `/setup`.", ephemeral=True)
            return
        ...
        for i, role_id in enumerate(top_roles[:3]):
            ...
            roles_text += (f"{labels[i]}: {role.mention if role else 'No encontrado'}\n")
        if not roles_text:
    ...
    # ── /set ───────────────────────────────────────────────────
    @app_commands.command(name="set", description="[Admin] Modificar un parámetro de configuración")
    @app_commands.default_permissions(administrator=True)
    ...
    async def set_param(self, interaction: discord.Interaction, parameter: str, value: str):
        guild_id = interaction.guild_id

        try:
            num_value, display_value = validate_param(parameter, value)
        except ValueError as e:
            await interaction.response.send_message(f"Valor inválido: {e}", ephemeral=True)
            return

        async with self.bot.db.acquire() as conn:
            await conn.execute("INSERT INTO guild_config (guild_id) VALUES ($1) ON CONFLICT DO NOTHING;", guild_id)
            await conn.execute(f"UPDATE guild_config SET {parameter} = $2, updated_at = NOW() WHERE guild_id = $1;", guild_id, num_value)

        embed = discord.Embed(title="Parámetro actualizado", description=f"**{parameter}** = `{display_value}`", color=discord.Color.green())
    ...
    async def give(self, interaction: discord.Interaction, member: discord.Member, amount: int, currency: str = "both"):
        ...
        async with self.bot.db.acquire() as conn:
            user = await conn.fetchrow("SELECT * FROM users WHERE user_id = $1 AND guild_id = $2", member.id, guild_id)
    ...
    @app_commands.default_permissions(administrator=True)
    @app_commands.describe(target="Qué resetear", member="Usuario específico (solo para 'Un usuario')")
    ...
    async def reset(self, interaction: discord.Interaction, target: str, member: discord.Member | None = None):
        guild_id = interaction.guild_id
        logger = self.bot.get_cog("LoggerCog")

        if target == "user":
            if not member:
                await interaction.response.send_message("Especifica un usuario con `member:`.", ephemeral=True)
                return

            view = ConfirmView(interaction.user.id)
            await interaction.response.send_message(f"¿Resetear TODOS los datos de **{member.display_name}**?", view=view, ephemeral=True)
            await view.wait()
            if not view.confirmed:
                return

            async with self.bot.db.acquire() as conn:
                await conn.execute("DELETE FROM answer_history WHERE user_id = $1 AND guild_id = $2;", member.id, guild_id)
            ...
            if logger:
                await logger.log_admin_reset(guild_id, interaction.user, "Usuario", member)
            ...
            if logger:
                await logger.log_admin_reset(guild_id, interaction.user, "Ranking completo")

        elif target == "jackpot":
            async with self.bot.db.acquire() as conn:
                await conn.execute("UPDATE gold_events SET jackpot = 0 WHERE guild_id = $1 AND winner_id IS NULL;", guild_id)

            await interaction.response.send_message("Jackpot de Pregunta de Oro reseteado a 0.", ephemeral=True)

            if logger:
                await logger.log_admin_reset(guild_id, interaction.user, "Jackpot de Oro")

        elif target == "cooldowns":
            async with self.bot.db.acquire() as conn:
                await conn.execute("UPDATE users SET last_daily = NULL, last_robbery = NULL, robberies_today = 0, updated_at = NOW() WHERE guild_id = $1;", guild_id)

            quiz_cog = self.bot.get_cog("QuizCog")
            if quiz_cog:
                quiz_cog._cooldowns.clear()

            await interaction.response.send_message("Todos los cooldowns reseteados.", ephemeral=True)

            if logger:
                await logger.log_admin_reset(guild_id, interaction.user, "Cooldowns")

        elif target == "all":
            view = ConfirmView(interaction.user.id)
            await interaction.response.send_message("**ATENCIÓN:** Esto borrará todos los datos del servidor:\nUsuarios, historial, robos, transacciones, eventos de Oro y roles temporales.\n\n**Esta acción es irreversible.**", view=view, ephemeral=True)
            await view.wait()
            if not view.confirmed:
                return

            async with self.bot.db.acquire() as conn:
                for table in ["answer_history", "transactions", "robberies", "temp_roles", "gold_events", "users"]:
            ...
            await interaction.followup.send("**Todos los datos del servidor han sido eliminados.**", ephemeral=True)

            if logger:
                await logger.log_admin_reset(guild_id, interaction.user, "TODO el servidor")

    # ── /sync ──────────────────────────────────────────────────
    @app_commands.command(name="sync", description="[Admin] Sincronizar comandos slash del bot")
    @app_commands.default_permissions(administrator=True)
    async def sync_commands(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(f"Error sincronizando: {e}", ephemeral=True)

    # ── /status ──────────────────────────────────────────────────
    @app_commands.command(name="status", description="[Admin] Estado del bot y estadísticas del servidor")
    @app_commands.default_permissions(administrator=True)
    async def status(self, interaction: discord.Interaction):
        guild_id = interaction.guild_id

        async with self.bot.db.acquire() as conn:
            total_users = await conn.fetchval("SELECT COUNT(*) FROM users WHERE guild_id = $1;", guild_id)
            active_users = await conn.fetchval("SELECT COUNT(*) FROM users WHERE guild_id = $1 AND updated_at > NOW() - INTERVAL '7 days';", guild_id)
            total_quizzes = await conn.fetchval("SELECT COUNT(*) FROM answer_history WHERE guild_id = $1;", guild_id)
            quizzes_today = await conn.fetchval("SELECT COUNT(*) FROM answer_history WHERE guild_id = $1 AND answered_at > NOW() - INTERVAL '1 day';", guild_id)
            total_robberies = await conn.fetchval("SELECT COUNT(*) FROM robberies WHERE guild_id = $1;", guild_id)
            total_gold = await conn.fetchval("SELECT COUNT(*) FROM gold_events WHERE guild_id = $1;", guild_id)
            jackpot = await conn.fetchval("SELECT COALESCE(SUM(jackpot), 0) FROM gold_events WHERE guild_id = $1 AND winner_id IS NULL AND is_active = FALSE;", guild_id)
            total_questions = await conn.fetchval("SELECT COUNT(*) FROM questions;")
            active_temp_roles = await conn.fetchval("SELECT COUNT(*) FROM temp_roles WHERE guild_id = $1 AND removed = FALSE AND expires_at > NOW();", guild_id)
        ...
        embed = discord.Embed(title=f"Estado del Bot — {interaction.guild.name}", color=discord.Color.blurple())
        embed.add_field(name="Usuarios", value=(f"Total: **{total_users}**\nActivos (7d): **{active_users}**"), inline=True)
        embed.add_field(name="Partidas", value=(f"Total: **{total_quizzes}**\nHoy: **{quizzes_today}**"), inline=True)
        embed.add_field(name="Robos / Oro", value=(f"Robos: **{total_robberies}**\nEventos Oro: **{total_gold}**\nJackpot: **{jackpot}**"), inline=True)
        embed.add_field(name="Base de datos", value=(f"Preguntas cacheadas: **{total_questions}**\nRoles temporales activos: **{active_temp_roles}**"), inline=True)
        embed.add_field(name="Bot", value=(f"Servidores: **{len(self.bot.guilds)}**\nCogs: **{len(cogs_loaded)}** ({', '.join(cogs_loaded)})\nLatencia: **{self.bot.latency * 1000:.0f}ms**\nUptime: **{hours}h {mins}m**"), inline=False)
